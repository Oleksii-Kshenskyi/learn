...
CLICommands::CLICommands(int argc, char** argv):
    ...
    commands["delete"] = [&](std::vector<std::string> myargs) { return todos.delete_(myargs); };
}
...
std::string CLICommands::run() {
    ...
    if(commands.count(args[1]) == 0) { return (std::stringstream() << "ERROR: " << args[1] << ": unknown command.").str(); }
